...
t = Label(
    window, text="Classification of Resume", fg="white", bg="black", font=("Times", 24)
)

t.pack(fill=X)
frame = Frame(window, bg="#c6cca5", bd=15)
frame2 = Frame(window, bg="#c6cca5", bd=15)
frame3 = Frame(window, bg="#c6cca5")


def open_report():
    path = folder_path+"\\report.txt"
    os.startfile(path)


def open_bar_graph():
    path = folder_path+"\\bar_chart.png"
    im = Image.open(path)
    im.show()


def open_pie_chart():
    path = folder_path+"\\pie_chart.png"
    im = Image.open(path)
    im.show()
...
def analyze():
    ...
    if file_path != "":
        ...
        plt.savefig(folder_path+"\\bar_chart.png")
        # Pie-chart
        targetCounts = (
            resumeDataSet["Category"].value_counts().reset_index()["Category"]
        )
        ...
        plt.savefig(folder_path+"\\pie_chart.png")
...
button_explore = Button(frame, text="Choose File", command=browseFiles, width=15)


button_bar = Button(frame2, text="Bar Graph", command=open_bar_graph, width=15)

button_pie = Button(frame2, text="Pie Chart", command=open_pie_chart, width=15)
button_report = Button(frame2, text="Report", command=open_report, width=15)
button_analyze = Button(
    frame3, text="Analyze", command=analyze, width=15, padx=10, pady=10
)

resume_dataset = Label(
    frame,
    text="Select Resume Dataset",
    fg="black",
    bg="#c6cca5",
...
result = Label(
    frame2, text="Results", fg="black", bg="#c6cca5", font=("Helvetica", 14, "bold")
)
